...
const MinimalistTemplate = ({
	...
	skills,
	isPdf = false,
}: TemplateCommonProps & { isPdf?: boolean }) => {
			...
			<div className="content-area px-12 py-8">
				{orderedTabs.map((tab, i) => {
					...
					if (React.isValidElement(section)) {
						const validSection = section as ReactElement<{ className?: string }>
						return React.cloneElement(validSection, {
							key: i,
							className: `${validSection.props.className || ""} ${
								isPdf ? "pdf-section" : ""
							}`,
						})
					}
